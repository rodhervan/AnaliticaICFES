...
# Cargar modelo
modelo = load_model("modelo_binarioRN2.h5")
...
layout_shap = html.Div([
    dbc.Card([
        dbc.CardBody([
            html.H5("Desempeño del Modelo", className="card-title text-white"),
            html.P("Precisión: 72.7%", className="card-text text-white mb-1"),
            html.P("Pérdida (Loss): 0.526", className="card-text text-white"),
        ])
    ], className="mb-4 shadow-sm bg-primary", style={"maxWidth": "400px", "border": "none"}),
...
app.layout = html.Div(style={'backgroundColor': '#e6f0fa', 'minHeight': '100vh'}, children=[
    ...
    dbc.NavbarSimple(
        children=[
            ...
            dbc.NavItem(dbc.NavLink("Sobre el modelo", href="/shap")),
        ],
